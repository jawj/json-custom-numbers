...
const shift = 7;
const transitionsCount = (acomma << shift) + closebrace + 1;
const tr: any[] = new Array(transitionsCount).fill(err);

// end of input
tr[NaN | ok << shift] = end;
[go, firstokey, okey, ocolon, ovalue, ocomma, firstavalue, avalue, acomma].forEach(state => {
  tr[NaN | state << shift] = err;
});

// whitespace
[space, tab, cr, newline].forEach(ch => {
  [go, ok, firstokey, okey, ocolon, ovalue, ocomma, firstavalue, avalue, acomma].forEach(state => {
    tr[ch | state << shift] = noop;
  });
});
...
Object.keys(actions).forEach(action => {
  ...
  const allch = chdesc === 'word' ? [f, t, n, minus, ...new Array(10).fill(undefined).map((_, i) => 48 + i)] : [chdesc];
  allch.forEach(ch => 
    states.forEach(state => 
      tr[(typeof ch === 'number' ? ch : charmap[ch as keyof typeof charmap]) | statemap[state as keyof typeof statemap] << shift] = actions[action as keyof typeof actions]));
});
...
export function parse(json: string) {
  ...
  do { ch = text.charCodeAt(at++); } while (!tr[ch | state << shift]());
  return value;
}
